...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    while placement.is_none() && attempts < 10 {
        ...
        dimensions.for_each(|pt| {
            ...
            let distance = dijsktra_map.map[idx];
            if distance < 2000.0 && distance > 20.0 && mb.amulet_of_yala_start != pt {
                can_place = true; // TODO: checking here seems weird, this should really short circuit if any placement is invalid
            }
        });

        if can_place {
            placement = Some(Point::new(dimensions.x1, dimensions.y1)); // TODO: weird way to guard a loop, ill fix that later
