...
pub fn hud(ecs: &SubWorld) {
    ...
    let player = <(Entity, &Player,)>::query().iter(ecs).find_map(|(entity, _player)| Some(*entity)).unwrap();
    ...
    if y > 4 {
        draw_batch.print_color(Point::new(0, 3), "Items carried", ColorPair::new(YELLOW, BLACK));
    } else {
        draw_batch.print_color(Point::new(0, 3), "Press 'G' to pickup items", ColorPair::new(YELLOW, BLACK));
    }
