...
pub struct Map {
    // `tiles` map index to index in `revealed_tiles`
...
impl Map {
    ...
    // returns `Some(index)`` if its a valid exit where `index` is tile destination else `None`
...
impl BaseMap for Map {
    fn get_available_exits(&self, idx: usize) -> SmallVec<[(usize, f32); 10]> {
        ...
        // return small vec (index of tile, weight)
